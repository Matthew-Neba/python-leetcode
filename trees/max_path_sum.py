...
# Key to this problem is realizing that to obtain the maximum path sum for the entire tree, we need to obtain the maximum path sum for the trees rooted at every node, i.e every subtree. Then, we can return the maximum of those maximum path sum. But, we notice that when calculating the maximum path sum for a subtree, we can use part of the calculation to help the parent calculate it's maximum path sum. This avoids repeated work and makes the time complexirt O(n).
class Solution:
